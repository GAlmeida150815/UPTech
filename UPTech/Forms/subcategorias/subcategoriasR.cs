...
namespace UPTech {
	public partial class subcategoriasR: Form {
		dbConn conn = new dbConn();
		Form parentForm;
		public subcategoriasR (Form parent) {
			InitializeComponent();
			parentForm = parent;
		}
		...
		private void spawnButtons () {
			string[] subcategoriasID = conn.sqlSelectStringArray($"SELECT * FROM subcategorias","id");
			string[] subcategorias = conn.sqlSelectStringArray($"SELECT * FROM subcategorias","subCategoria");

			for (int i = 0;i < subcategoriasID.Length;i++) {
				...
				but.Text = $"{subcategoriasID[i]} - {subcategorias[i]}";
		...
		private void visualizeInfos(object sender, EventArgs e) {
			...
			openChildForm(new subcategoriaR(parentForm,id));
		}
