...
namespace UPTech {
	public partial class compras: Form {
		...
		private void spawnButtons () {
			int[] comprasID = conn.sqlSelectIntArray($"SELECT * FROM orders","id");

			for (int i = 0;i < comprasID.Length;i++) {
				int id = conn.sqlSelectInt($"SELECT * FROM orders WHERE id = {comprasID[i]}", "idUser");
				string email = conn.sqlSelectString($"SELECT * FROM users WHERE id = {id}", "email");
		...
		private void visualizeInfos(object sender, EventArgs e) {
			...
			openChildForm(new compra(parentForm,id));
		}
