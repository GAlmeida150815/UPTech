		...
		private void btnInserir_Click (object sender, EventArgs e) {
			...
			if (txtEmail.Text != "") {
				if (isEmailValid(txtEmail.Text)) {
					if (!conn.sqlSelect($"SELECT * FROM users WHERE email = '{txtEmail.Text}'")) {
						if (txtPassword.Text != ""){
							if (id == 0) {
						...
						MetroFramework.MetroMessageBox.Show(parentForm, "Já existe um utilizador registado com esse email", "Erro a inserir a categoria", 100);
						return;
					}
				} else {
