...
namespace UPTech {
	public partial class utilizadoresR: Form {
		dbConn conn = new dbConn();
		Form parentForm;
		public utilizadoresR (Form parent) {
			InitializeComponent();
			parentForm = parent;
		}
		...
		private void spawnButtons () {
			string[] usersID = conn.sqlSelectStringArray($"SELECT * FROM users","id");
			string[] users = conn.sqlSelectStringArray($"SELECT * FROM users","email");
		...
		private void visualizeInfos(object sender, EventArgs e) {
			...
			openChildForm(new utilizadorR(parentForm,id));
		}
