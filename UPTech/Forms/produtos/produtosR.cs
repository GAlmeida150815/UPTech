...
namespace UPTech {
	public partial class produtosR: Form {
		dbConn conn = new dbConn();
		Form parentForm;
		public produtosR (Form parent) {
			InitializeComponent();
			parentForm = parent;
		}
		...
		private void spawnButtons () {
			string[] productsID = conn.sqlSelectStringArray($"SELECT * FROM produtos","id");
			...
			for (int i = 0;i < productsID.Length;i++) {
				...
				but.Text = $"{productsID[i]} - {products[i]}";
		...
		private void visualizeInfos(object sender, EventArgs e) {
			...
			openChildForm(new produtoR(parentForm,id));
		}
